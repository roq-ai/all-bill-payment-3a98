...
import { useMerchantFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['finance-manager', 'business-owner', 'sales-representative', 'customer-service-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useMerchantFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Merchants',
      path: isTenantUser ? `/merchants/view/${data?.id}` : '/merchants',
      entity: 'merchant',
